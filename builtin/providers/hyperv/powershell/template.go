...
type executeElevatedCommandTemplateOptions struct {
	Vars            string
	Path        	string
}

var executeElevatedCommandTemplate = template.Must(template.New("ExecuteElevatedCommand").Parse(`if (Test-Path variable:global:ProgressPreference){$ProgressPreference='SilentlyContinue'};{{.Vars}}&'{{.Path}}';exit $LastExitCode`))
